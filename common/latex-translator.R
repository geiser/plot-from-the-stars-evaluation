...
write_rel_analysis_in_latex <- function(
  fa_mod, cfa_mod, alpha_mods, in_title = "none", filename = "RelAnalysis.tex", key_labels = list(), append = F) {
  ...
  cfa_summary <- paste0(
    c("\\raggedleft{", "\\scriptsize{"
      , paste0(c(paste0("CFI: ", fit_measures$cfi)
                 , paste0("TLI: ", fit_measures$tli)
                 , paste0("\\\\ ", "df: ", fit_measures$df)
                 , paste0("$\\chi^2$: ", fit_measures$chisq)
                 , paste0("p-value: ", fit_measures$pvalue)
                 , paste0("\\\\ ", "RMSEA: ", paste0(fit_measures$rmsea, " "
